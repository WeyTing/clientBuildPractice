...
<template>
  <nav
    class="relative md:h-16 h-14 w-full bg-black flex items-center justify-between"
  >
      ...
      <div>
        ...
        <button type="button" class="ml-1" @click="toggleEdit">
          <img
            :src="Edit"
            alt="editBtn"
            class="w-[13px] h-[13px] hover:cursor-pointer"
          />
        </button>
        <div>
    ...
    <div class="flex items-center gap-2 mr-3">
      <button
        v-if="isLoggedIn"
        type="button"
        class="text-[aliceblue] rounded px-3 md:px-5 py-1 md:py-2 bg-[#444857] editorSmallButton-hover-bgc hover:cursor-pointer"
      >
        <div class="h-7 flex">
      ...
      <button
        v-if="!currentWork.isAutoPreview"
        type="button"
        class="text-[aliceblue] rounded-l px-5 py-2 bg-[#444857] mr-[1px] editorSmallButton-hover-bgc hover:cursor-pointer"
        @click="runPreview"
      >
      ...
      <div class="md:flex hidden">
        ...
        <div class="relative">
          ...
          <ul
            v-if="saveOptionVisible"
            class="absolute z-50 flex flex-col rounded-sm top-12 right-0 bg-[#2C303A] text-white w-80 justify-around border-4 border-gray-800 px-5"
          >
            <label
              class="flex py-2 justify-between border-b border-gray-600 hover:cursor-pointer"
            >
              <span>Private</span>
              <div>
      ...
      <button
        ...
        type="button"
        class="flex md:hidden text-[aliceblue] rounded px-2 py-1 bg-[#444857] editorSmallButton-hover-bgc hover:cursor-pointer"
      >
        <div class="h-7 flex justify-between w-6 items-center">
          <div
            class="transition-transform h-0.5 bg-gray-200 relative before:content-[''] before:w-1.5 before:h-0.5 before:bg-gray-200 before:absolute before:-top-1.5 before:left-0 after:content-[''] after:w-3.5 after:h-0.5 after:bg-gray-200 after:absolute after:-bottom-1.5 after:left-0"
            :class="navListVisible ? 'before:w-2 w-1.5' : 'before:w-1.5 w-2.5'"
          ></div>
          <img
            :src="ArrowWhite"
            alt="arrowWhite"
            class="transition-transform w-3 self-start mt-1.5"
            :class="{ 'scale-y-[-1]': navListVisible }"
          />
        </div>
      </button>
      <div
        v-if="navListVisible"
        class="z-50 absolute flex flex-col top-14 right-0 w-55 gap-1 py-1 bg-[#1E1F26] rounded-sm"
      >
        <button
          class="flex w-full px-2 py-1 hover:bg-gray-500"
          @click.prevent="handleSave"
        >
          <img :src="Cloud" alt="saveBtn" class="w-4" />
          <span>Save</span>
      ...
      <button
        @click.prevent="toggleSetting"
        type="button"
        class="hidden md:flex text-[aliceblue] rounded px-4 py-2 bg-[#444857] editorSmallButton-hover-bgc hover:cursor-pointer"
      >
        <div class="h-7 flex items-center gap-1">
          <img :src="Settings" alt="settingBtn" class="w-4" />
      ...
      <div class="relative md:flex hidden">
        <button
          ...
          class="text-[aliceblue] rounded px-4 py-2 bg-[#444857] editorSmallButton-hover-bgc hover:cursor-pointer"
        >
          <div class="h-7 flex items-center gap-1">
            <img
              :src="Layout"
              alt=""
              class="w-[14px]"
              :style="{ transform: `rotate(${selectedLayout.rotation}deg)` }"
            />
          </div>
        </button>
        <div
          v-if="layoutOptionVisible"
          class="fixed inset-0 transition-opacity duration-200"
          @click="toggleLayout"
        ></div>
        <div
          v-if="layoutOptionVisible"
          class="absolute z-50 bg-[#2C303A] top-12 right-0 py-3 rounded-lg border-4 border-gray-800"
        >
          ...
          <div class="flex justify-center align-middle py-3">
            <div class="flex justify-center align-middle py-3">
              <label
                ...
                :key="option.id"
                class="border-2 border-[#444857] w-20 flex justify-center h-12 editorSmallButton-hover-bgc hover:cursor-pointer"
                :class="{
                  'rounded-l-sm': option.id === 'left',
                  'rounded-r-sm': option.id === 'right',
                  'bg-[#444857]': selectedLayout.id === option.id,
                }"
              >
                <button
          ...
          <ul
            class="relative flex flex-col rounded-sm right-0 bg-[#2C303A] text-white w-65 justify-between"
          >
      ...
      <button
        v-if="!isLoggedIn"
        type="button"
        class="text-black rounded px-2 py-1 md:px-4 md:py-2 bg-[#47cf73] hover:cursor-pointer"
      >
        <div class="h-7 flex items-center gap-1">
          <span>Sign Up</span>
        </div>
      ...
      <button
        v-if="!isLoggedIn"
        type="button"
        class="text-[aliceblue] rounded px-2 py-1 md:px-4 md:py-2 bg-[#444857] hover:cursor-pointer"
      >
        <div class="h-7 flex items-center gap-1">
          <span>Log In</span>
        </div>
      ...
      <div
        v-if="isLoggedIn"
        class="w-9 h-9 md:w-11 md:h-11 overflow-hidden mx-1 rounded hover:cursor-pointer"
      >
        <img
          src="https://fakeimg.pl/300x200/500"
